...
module ImageWrangler
  class C2paTest < Minitest::Test
    def setup
      ...
      @described_class = ImageWrangler::C2pa
    end

    def test_raises_error_when_command_missing
      assert_raises ImageWrangler::Error do
        @described_class.new(@c2pa_camera, {command: "nonexistent_command"})
      end
    end

    def test_extraction_when_no_c2pa
      subject = @described_class.new(@composite)
      assert_equal({}, subject.manifests)
    end

    def test_present_with_c2pa
      subject = @described_class.new(@c2pa_camera)
    ...
    def test_empty_file
      assert_raises ImageWrangler::Error do
        @described_class.new(@empty_file)
      end
    end

    def test_non_image_file
      subject = @described_class.new(@non_image_file)
      refute subject.present?
    end

    def test_invalid_c2pa
      subject = @described_class.new(@c2pa_invalid)
      refute subject.valid?
    end

    def test_valid_without_c2pa
      subject = @described_class.new(@no_c2pa)
